# Internal context repository

Nonpublic, nonpublished content for agents and employees.

## Primary writing reference

For content drafting and editing, use:

- `internal-contexts/writing style guides/jack-writing-style-guide.md` (default)

Secondary reference:

- `internal-contexts/writing style guides/deadwater-blog-style-guide.md` (punctuation and formatting conventions)

Planned additions: writing samples, product descriptions, case studies, internal briefs, and other reference material.
